...
export class RoutesDriverService {
    ...
    async createOrUpdate(dto: {route_id: string, lat: number, lng: number}) {
        ...
        const lastPoint = directions.routes[0].legs[0].steps[directions.routes[0].legs[0].steps.length - 1]
        if (lastPoint.end_location.lat === dto.lat && lastPoint.end_location.lng === dto.lng) {
            await this.kafkaProducerQueue.add({
                event: 'RouteFinished',
                id: routeDriver.route.id,
                name: routeDriver.route.name,
